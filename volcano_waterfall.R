...
tumor.probe.names = c("Beta.Catenin","pS6","PTEN","P.ERK","S6","Ki.67", "Beta.2.microglobulin","AKT",
                      "PSTAT3","p.AKT","Her2", "Pan.Cytokeratin")
immune.probe.names = c("CD8","CD4","CD68","CD14","GZMB","CD3","CD66B","VISTA","PD1","PD.L1","IDO.1",
                       ...
                       "CD27","CD163","OX40L.CD252.TXGP1","Bcl.2","B7.H3","B7.H4.VTCN1", "Mouse.IgG1", "IgG.Rabbit.Isotype")
all.probes = c(tumor.probe.names, immune.probe.names)

# example code used to generate waterfall plots (e.g. figures 2d, 2e, and 5a)
waterfall.plot <- function(pcr, point, roi_type, er.status,ylab) {
  df_rna <- gather(data.norm, key='Gene',value='count', 9:(length(colnames(data.norm))))
                   # the 9 is the first column of protein data, should be modified accordingly
  genes=unique(df_rna$Gene)
  df_rna_m <- melt(df_rna, id.vars = c('dsp_roi_uid','patient','timepoint','panck','no.pCR','ER', 'Gene'), 
                   measure.vars = 'count')
  df_t_DE <- subset(df_rna_m,  timepoint %in% point & panck %in% roi_type & no.pCR %in% pcr & ER %in% er.status)
...
er.status <- c(1)
filename = 'output.pdf'
xlab = expression('Change Runin/Baseline, log'[2]*'(PanCK-E)')
...
volcano.plot <- function(pcr, point, roi_type, er.status,xlab) {
  df_rna <- gather(data.norm, key='Gene',value='count', 25:(length(colnames(data.norm))-1))
  genes=unique(df_rna$Gene)
  df_rna_m <- melt(df_rna, id.vars = c('dsp_roi_uid','patient','timepoint','panck','no.pCR','ER', 'Gene'), 
                   measure.vars = 'count')
  df_t_DE <- subset(df_rna_m,  timepoint %in% point & panck %in% roi_type & no.pCR %in% pcr & ER %in% er.status)
